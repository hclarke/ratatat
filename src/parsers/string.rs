...
impl<'a> Generator<'a> for EscapeChar {
	...
	fn generate(ctx: &Context<'a>) -> Rc<DynParser<'a, Self::O>> {
		...
		let esc_simple = ch.clone().
			filter_map(move |c| esc_map.get(&c).map(|v| *v));

		let esc_hex = ('x', Digit(16).many(2..=2))
			.filter_map(|(_,digits)| {
				let val = (digits[0] << 4) | digits[1];
				core::char::from_u32(val as u32)
			});

		let esc_unicode = ("u{", Digit(16).many(1..=6), '}')
			.filter_map(|(_, digits, _)| {
				let mut val = 0;
				for d in digits {
					val *= 16;
					val += d as u32;
...
mod test {
    ...
    #[test]
    fn string_parser() {
        assert_parse!(Some("hello".to_string()), parser::<String>(), r##""hello""##);
        assert_parse!(Some("hey\nworld".to_string()), parser::<String>(), r##""hey\nworld""##);
